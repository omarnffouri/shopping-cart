...
<script setup>
  ...
  const initZoom = async () => {
    await checkImageLoaded(image.value);
    await checkImageLoaded(zoomImage.value);
    img = imageRef.value;
  ...
  onBeforeUnmount(() => {
    window.removeEventListener("resize", calcDimension);
  });

  watch([image, zoomImage, zoomFactor], async () => {
    await nextTick();
    await initZoom();
  });
