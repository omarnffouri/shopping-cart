...
<script setup>
  ...
  const valueChanged = async ({bundleDeal, product, inventory, direction, note = null}) => {
    let apiVal = {
    ...
    if (note && (note.message || note.image)) {
      const form = new FormData();
      form.append('user_token', await getUserToken());
      form.append('product_id', product.id);
      form.append('inventory_id', inventory.id);
      form.append('quantity', direction);
      if (note.message) {
