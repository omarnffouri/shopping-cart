...
export default {
    setup(){
        ...
        return {getUserToken, currencyPosition, langCode, profile,
            postRequest, currencyIcon, setting, setToastMessage, setToastError
        }
    },
    name: 'StripePayment',
    data() {
    ...
    methods: {
        ...
        createStripeToken() {
            ...
            this.stripe
                .createToken(this.card, { name: this.profile?.name})
                .then(async result => {
                    if (result.error) {
