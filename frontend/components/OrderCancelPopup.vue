...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      if (this.title && this.cancellationMessage) {
        ...
        // Prepare params - different for admin vs user
        const params = {
          order_id: this.orderId,
          message: this.cancellationMessage,
          title: this.title,
        };
        params.user_token = await this.getUserToken();
  ...
  async mounted() {
    ...
    // Try to fetch existing cancellation data
    const params = `/${this.orderId}?${prepareGetUrl({
          id: this.orderId,
