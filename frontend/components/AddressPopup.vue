...
<script>
  ...
  export default {
    setup(){
      ...
      const {fetchLocation, setToastMessage, setToastError, getRequest, postRequest, fetchStateDeliveryTypes} = commonStore

      return {getUserToken, countryList, phoneList, langCode, setCountryList, location,
        fetchLocation, setToastMessage, setToastError, getRequest, postRequest, setAllAddress, updateAddress,
        setPhoneList, userAddressAction, profile, fetchStateDeliveryTypes}
    },
    ...
    computed: {
      ...
      countryLabel() {
        if (!this.addressData?.country) return '';
        return this.countryList?.[this.addressData.country]?.name || this.addressData.country;
      },
      editing() {
    ...
    methods: {
      ...
      settingCountry() {
        this.addressData.country = this.addressData.country.trim() !== '' ? this.addressData.country.trim() : (this.location.countryCode || 'AE')
        const stateValues = Object.values(this.states || {});
        const currentState = `${this.addressData?.state || ''}`.trim();
        if (currentState) {
          this.addressData.state = currentState;
          return;
        }
        const locationState = `${this.location.region || ''}`.trim().toLowerCase();
        ...
        this.addressData.state = matchedState?.code || stateValues?.[0]?.code || '';
      },
      selectedState(evt) {
        this.addressData.state = evt.value.code
      },
      async loadStates() {
        const mappedStates = await this.fetchStateDeliveryTypes({lang: this.langCode});
