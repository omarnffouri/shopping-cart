...
export function useAddressHelper({submittingAddressData, hasAddressErrors, ajaxDeleting}) {
    ...
    const addressAction = async (addressData) => {
        if (addressData.value.email && addressData.value.phone && addressData.value.name && addressData.value.country && addressData.value.address_1) {
            addressData.value.city = countryList.value[addressData.value.country].states[addressData.value.state].name;
            addressData.value.zip = "00000";
