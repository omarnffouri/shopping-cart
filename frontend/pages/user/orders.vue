<template>
  <client-only>
    <account-layout active-route="orders" @clicked-orders="loadData" class="mb-5">
      <template v-slot:rightArea>
        ...
        <order-tabbing ref="orderTabRef"/>
        <div class="spinner-wrapper flex layer-white" v-if="fetchingOrderData">
          <spinner :radius="100"/>
        </div>
        <div v-else-if="currentOrders && !currentOrders.length" class="info-msg">
          {{ $t('orders.noOrderYet') }}
        </div>
        <div v-else>
          <div v-for="(value, index) in currentOrders" :key="index" class="card mb-15">
            <div class="flex sided b-b ptb-10 plr-20 plr-sm-15 block-xs">
              <div>
                <nuxt-link :to="`${orderBasePath}/${value.id}`" class="block">
                  {{ $t('order.order') }}
              ...
              <div>
                <nuxt-link :to="`${orderBasePath}/${value.id}`" class="link-color mt-xs-5">
                  {{ $t('orders.manageOrder') }}
                </nuxt-link>
              </div>
            </div>

            <div v-for="(ordered, i) in value.ordered_products" :key="i"
                 class="flex sided ptb-10 plr-20 plr-sm-15">
              <div class="flex grow gap-15">
                <div class="w-80x">
                  <nuxt-link :to="productLink(ordered.product)" class="img-wrapper w-100"
                ...
                <div class="flex grow sided block-xs">
                  ...
                  <div class="flex start">
                    ...
                    <h5 class="">
                      ...
                      <price-format :price="parseInt(ordered.selling)"/>
                    </h5>
                  </div>
                </div>
              </div>
        ...
        <div class="flow-hidden">
          <pagination v-if="!changedSelectedOrder" ref="orderPaginationRef" :total-page="totalPage"/>
        </div>
      </template>
    </account-layout>
  </client-only>
...
const totalPage = computed(() => orderedList.value?.last_page);
const currentOrders = computed(() => orderedList.value?.data);
...
const loadData = () => {
  orderPaginationRef.value?.routeParam();
};
...
const fetchingData = async () => {
  ...
  const params = {
    ...
    is_paid: props.orderBasePath.includes("/user/order"),
