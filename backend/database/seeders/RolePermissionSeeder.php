...
//            ]
        ...
        foreach ($permissions as $permGroup) {
            $groupName = $permGroup['group_name'];
            foreach ($permGroup['permissions'] as $permName) {
                $permission = Permission::firstOrCreate(
                    ['name' => $permName, 'guard_name' => 'admin'],
                    ['name' => $permName, 'guard_name' => 'admin', 'group_name' => $groupName]
                );

                // Super admin has all permission except create withdrawal request
                if($permName !== 'withdrawal_request.create')
                {
                ...
                /*if(
                    ...
                    $permName !== 'store.delete' &&
                    $permName !== 'store.create' &&

                    $permName !== 'withdrawal_request.delete' &&
                    $permName !== 'withdrawal_request.approve'
                ){
