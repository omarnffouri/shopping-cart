...
class AdminsController extends Controller
{
    ...
    public function statistic(Request $request)
    {
        try {
            ...
            if ($posLicenceValid && $posPlugin && $posPlugin->active) {
                $categoryQuery[] = 'o.pos_order_id';
                $brandSelect[] = 'o.pos_order_id';
                $productSelect[] = 'o.pos_order_id';
            }
    ...
    public function login(Request $request)
    {
        try {
            ...
            $admin->load('roles');
            //return response()->json($admin->load('roles')); //for testing purpose
            $roleName = $admin->roles->first()?->name ?? null;
            //return response()->json( $roleName = $admin->roles->first()?->name ?? null);  //for testing purpose
            $data['admin'] = [
                'id' => $admin->id,
                'name' => $admin->name,
                'email' => $admin->email,
            ...
            //return response()->json($data); //for testing purpose
            return response()->json(new Response($request->token, $data));
    ...
    public function profile(Request $request): JsonResponse
    {
        try {
            ...
            $data['media_storage'] = config('env.media.STORAGE');
        ...
        try {
            ...
            if ($admin->id) {
                ...
                if ($changedActivation) {
                    ...
                    if ($adminUrl == config('env.url.CLIENT_BASE_URL')) {
    ...
    {
        try {
            $url = config('env.license_server') . "/api/activate-ishop/{$request->code}";
