...
class ProductsController extends ControllerHelper
{
    ...
    public function all(Request $request)
    {
        try {
            ...
            if ($lang) {
                ...
                $query = $query->with([
                    'tax_rules' => function ($query) use ($lang) {
                        $query->leftJoin(
                            'tax_rule_langs as trl',
                            function ($join) use ($lang) {
                ...
                $query = $query->with([
                    'product_categories' => function ($query) use ($lang) {
                        $query->with([
                ...
                $query = $query->with([
                    'product_inventories' => function ($query) use ($lang) {
                        $query->with([
                            'inventory_attributes' => function ($query) use ($lang) {
                                $query->with([
                                    'attribute_value' => function ($query) use ($lang) {
                                        $query->with([
                ...
                if ($request->has('available_for_delivery_today')) {
                    $query->where('products.available_for_delivery_today', $request->available_for_delivery_today);
                }
    ...
    public function dropDownData(Request $request)
    {
        ...
        try {
            ...
            if ($lang) {
                ...
                $res['categories'] = Category::leftJoin(
                    'category_langs as cl',
                    function ($join) use ($lang) {
                        $join->on('categories.id', '=', 'cl.category_id');
                        $join->where('cl.lang', $lang);
                ...
                $res['tax_rules'] = TaxRules::leftJoin(
                    'tax_rule_langs as trl',
                    function ($join) use ($lang) {
                        $join->on('tax_rules.id', '=', 'trl.tax_rule_id');
                        $join->where('trl.lang', $lang);
                ...
                $res['shipping_rules'] = ShippingRule::leftJoin(
                ...
                $res['product_collections'] = ProductCollection::leftJoin(
                ...
                $res['bundle_deals'] = BundleDeal::leftJoin(
                    'bundle_deal_langs as bdl',
                    function ($join) use ($lang) {
                ...
                $res['attributes'] = Attribute::with([
                    'values' => function ($query) use ($lang) {
                        $query->leftJoin(
                            'attribute_value_langs as avl',
                            function ($join) use ($lang) {
                                $join->on('attribute_values.id', '=', 'avl.attribute_value_id');
    ...
    public function multipleImageUpload(Request $request, $productId)
    {
        try {
            ...
            if (config('env.media.STORAGE') == config('env.media.URL')) {
                ...
                $product_image['admin_id'] = $this->user->id;
    ...
    public function bulkUpdate(Request $request)
    {
        try {
            ...
            if ($action == 'available_for_delivery_today') {
                Product::whereIn('id', $ids)->update(['available_for_delivery_today' => $value]);
            }
