...
class FrontendController extends Controller
{
        ...
        try {
            ...
            if ($lang) {
                ...
                if ($sidebarData) {
                    ...
                    $allCollection = ProductCollection::leftJoin(
                        'product_collection_langs as pcl',
                    ...
                    $allShipping = ShippingRule::leftJoin(
                        'shipping_rule_langs as srl',
                        function ($join) use ($lang) {
    ...
    public function addons(Request $request)
    {
        try {
            ...
            $categories = Category::query()
                ->when($lang, function ($q) use ($lang) {
                    $q->leftJoin('category_langs as cl', function ($join) use ($lang) {
                        ...
                        ->select('categories.id', 'categories.slug', 'categories.image', 'cl.title');
                }, function ($q) {
                    $q->select('categories.id', 'categories.slug', 'categories.image', 'categories.title');
                })
                ->where('categories.status', Config::get('constants.status.PUBLIC'))
                ->when($categoryId && $categoryId !== 'all', fn ($q) => $q->where('categories.id', $categoryId))
                ->get();
            ...
            $pivot = ProductCategory::whereIn('category_id', $categoryIds)->get();

            $productIds = $pivot->pluck('product_id')->unique();

            if ($productIds->isEmpty()) {
                $categories->each(fn ($c) => $c->products = []);
                return response()->json(new Response($request->token, $categories));
            }

            $products = Product::query()
                ->whereIn('products.id', $productIds)
                ->where('products.status', Config::get('constants.status.PUBLIC'))
                ->where('products.allow_note', false)
                ->where('products.allow_note_image', false)
                ->with([
                    'product_inventories.inventory_attributes.attribute_value.attribute' => function ($q) use ($lang) {
                        if ($lang) {
                            $q->leftJoin('attribute_langs as al', function ($join) use ($lang) {
                ...
                ->when($lang, function ($q) use ($lang) {
                    $q->leftJoin('product_langs as pl', function ($join) use ($lang) {
                        $join->on('pl.product_id', 'products.id')
                            ->where('pl.lang', $lang);
                    })
                        ->select('products.id', 'pl.title', 'products.slug', 'products.image', 'products.selling', 'products.offered');
                }, function ($q) {
                    $q->select('products.id', 'products.title', 'products.slug', 'products.image', 'products.selling', 'products.offered');
                })
                ->get()
                ->keyBy('id');
            ...
            foreach ($pivot as $p) {
                if (isset($products[$p->product_id])) {
                    $categoryProducts[$p->category_id][] = $products[$p->product_id];
                }
            }

            foreach ($products as $product) {

                $product->inventory = $product->product_inventories;
                unset($product->product_inventories);

                $attributes = [];

                foreach ($product->inventory as $inv) {
                    foreach ($inv->inventory_attributes ?? [] as $ia) {

                        if (!$ia->attribute_value?->attribute) continue;

                        $attr = $ia->attribute_value->attribute;
                        $val  = $ia->attribute_value;

                        $attributes[$attr->id]['id'] = $attr->id;
                        $attributes[$attr->id]['title'] = $attr->title;
                ...
                $product->attribute = array_values($attributes);
            }

            $categories->each(function ($cat) use ($categoryProducts) {
                $cat->products = $categoryProducts[$cat->id] ?? [];
            });

            $categories = $categories->filter(fn($cat) => !empty($cat->products))->values();
        ...
        try {
            ...
            if ($lang) {
                ...
                if ($request->q) {
                    ...
                    if ($request->home_spm) {
                        ...
                        $source = HomeSlider::with('source_brands.brand')
                            ...
                            ->with('source_products.product')// ->where('banners.id', $request->banner)->first()
                ...
                $query = $query->select([
                    ...
                    'flash_sale_products.price',
                    'flash_sales.end_time',
                    'products.available_for_delivery_today'
                ]);
    ...
    public function common(Request $request)
    {
        try {
            ...
            $commonData = Utils::cacheRemember($cacheKey, function () use ($request, $lang) {
                ...
                $data['thumb_prefix'] = config('constants.media.THUMB_PREFIX');
                $data['default_image'] = config('constants.media.DEFAULT_IMAGE');
                ...
                if ($lang) {
                    ...
                    $query = $query->with([
                        'in_footer_child' => function ($query) use ($lang) {
            ...
            $homeData = Utils::cacheRemember($cacheKey . $lang, function () use ($request, $lang) {
                if ($lang) {
                    ...
                    $flashSales = FlashSale::with([
                        'products' => function ($query) use ($lang) {
                            $query->with([
                                'product_data' => function ($query) use ($lang) {
                                    $query->leftJoin(
                                        'product_langs as avl',
                                        function ($join) use ($lang) {
    ...
    public function product(Request $request, $id)
    {
        try {
            ...
            if (!$lang) {
                ...
                if ($lang) {
                    ...
                    $query = $query->with([
                        ...
                        ->with([
                            'bundle_deal' => function ($query) use ($lang) {
                        ...
                        ->with([
                            'shipping_rule' => function ($query) use ($lang) {
                    ...
                    $data['vouchers'] = Voucher::where('end_time', '>=', $currentTime)
                        ...
                        ->where('status', Config::get('constants.status.PUBLIC'))
                        ->select('title', 'price', 'type', 'code', 'min_spend', 'usage_limit', 'limit_per_customer', 'show_validity_date')
                        ->get();
                    ...
                    $primaryCategory = $pcQuery
                        ->with([
                            'category.parent_data' => function ($q) use ($lang) {
                        ...
                        ->with([
                            'category' => function ($pcQuery) use ($lang) {
                    ...
                    $currentCategories = $currentCategories->leftJoin(
                        'category_langs as cl',
                        function ($join) use ($lang) {
                            $join->on('cl.category_id', '=', 'categories.id');
                            $join->where('cl.lang', $lang);
                        }
                ...
                if (!empty($data['product_image_names']) && count($data['product_image_names']) > 0) {
            ...
            $data['note'] = [
                'message' => (bool) $data->allow_note,
                'image' => (bool) $data->allow_note_image
            ];

            unset($data['allow_note']);
            unset($data['allow_note_image']);
    ...
    public function trackOrder(Request $request)
    {
        ...
        try {
            ...
            $query = Order::with(['ordered_products.shipping_place','cancellation'])
    ...
    public function flashSale(Request $request, $id = null)
    {
        try {
            ...
            if (!is_null($id)) {
                ...
                if ($lang) {
                    $flashSaleQuery = $flashSaleQuery->leftJoin(
                        'flash_sale_langs as cl',
                        function ($join) use ($lang) {
                            $join->on('cl.flash_sale_id', '=', 'flash_sales.id');
                            $join->where('cl.lang', $lang);
                        }
                ...
                if ($lang) {
                    $flashSaleQuery = $flashSaleQuery->with([
                        'products' => function ($query) use ($lang) {
                            $query->with([
                                'product_data' => function ($query) use ($lang) {
                                    $query->leftJoin(
