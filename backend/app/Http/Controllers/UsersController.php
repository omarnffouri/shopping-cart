...
class UsersController extends ControllerHelper
{
    ...
    public function login(Request $request)
    {
        try {
            ...
            Auth::login($user);
//return response()->json("user verification ",$user->verified, "user activ status ", $user->active);
            if (!$user->verified) {
            ...
            $data['expires_in'] = Carbon::now()
                ->addDays(request('remember_token') ? 365 : Config::get('constants.auth.EXPIRATION_IN_DAYS'));
    ...
    public function addressAction(Request $request)
    {
        try {
            ...
            if ($request->user_token && !$request->user('user')) {
                $user_token = $request->user_token ?? Str::uuid()->toString();
                $guestUser = GuestUser::where('user_token', $user_token)->first();
                if (!$guestUser) {
