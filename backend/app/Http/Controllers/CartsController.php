...
use App\Models\CartProductNote;
...
class CartsController extends ControllerHelper
{
    ...
    public function byUser(Request $request)
    {
        try {
            ...
            $query = $query->with('note:id,cart_id,image,message');
            $query = $query->select('id', 'product_id', 'user_id', 'inventory_id', 'quantity',
                'selected', 'shipping_place_id', 'shipping_type');
            $data = $query->get();

            $data->transform(function ($cart) {
                $allowNote = $cart->flash_product
                    && ($cart->flash_product->allow_note || $cart->flash_product->allow_image_note);

                if ($allowNote && $cart->note) {
                    $cart->note = [
    ...
    public function buyNow(Request $request)
    {
        try {
            ...
            $noteValidation = $this->validateImageNotes($request, $product, $lang);

            if ($noteValidation->fails()) {
                response()->json(Validation::error($request->token,
                    $noteValidation->errors()->first()
                ));
            }
            ...
            if ($existingCart) {
                ...
                $cart = $this->getExistingCart($request, $existingCart, $product);
                ...
                $cart = $this->getCart($request, $product);
            }
    ...
    private function validateImageNotes(Request $request, $product, $lang)
    {
        $rules = [];
        $messages = [];

        if ($product && $product->allow_note) {
            $rules['message'] = 'required|string';
            $messages['message.required'] = __('lang.note_message_required', [], $lang);
        }

        if ($product && $product->allow_note_image) {
            $imageRules = $request->hasFile('image')
                ? 'required|file|mimes:jpg,jpeg,png,pdf|max:2048'
                : 'required|string';

            $rules['image'] = $imageRules;
            $messages['image.required'] = __('lang.note_image_required', [], $lang);
            $messages['image.file'] = __('lang.note_image_required', [], $lang);
            $messages['image.mimes'] = __('lang.note_image_invalid_type', [], $lang);
            $messages['image.max'] = __('lang.note_image_max_size', [], $lang);
        }
     ...
     * @return Cart
     */
    public function getCart(Request $request, $product): Cart
    {
        $cart = Cart::create($request->all());

        if ($product && ($product->allow_note || $product->allow_note_image)) {
            $noteData = ['cart_id' => $cart->id];

            if ($product->allow_note && $request->filled('message')) {
                $noteData['message'] = $request->message;
            }

            if ($product->allow_note_image && $request->hasFile('image')) {
                $image_info = FileHelper::uploadImage($request->file('image'), 'product');
                $noteData['image'] = $image_info['name'];
            }

            if (isset($noteData['message']) || isset($noteData['image'])) {
                CartProductNote::create($noteData);
            }
        }
        return $cart;
    }
     ...
     * @return Cart
     */
    public function getExistingCart(Request $request, Cart $existingCart, $product): Cart
    {
        $cart = $existingCart;
        ...
        if ($product && ($product->allow_note || $product->allow_note_image)) {
            ...
            if ($request->hasFile('image')) {
                $image_info = FileHelper::uploadImage($request->file('image'), 'product-note');
                $noteData['image'] = $image_info['name'];
            }

            if (isset($noteData['message']) || isset($noteData['image'])) {
                CartProductNote::updateOrCreate(
                    ['cart_id' => $cart->id],
                    $noteData
                );
            }
