...
class Utils
{
    public static function isUploadable($url): bool
    {
        return self::isImageUrl($url) && config('env.media.STORAGE') != config('env.media.URL');
    }

    public static function isImageUrl($url): bool
    {
    ...
    public static function copyImageFromUrl($url, $prefix = null)
    {
        ...
        if ($image_content !== false) {
            ...
            // Remember to delete the temporary file after you are done
            unlink($tmpfname);
            unset($file);
    ...
    public static function findCommonElements($arrays)
    {
        ...
        return array_intersect($baseArray, ...$arrays);
    }
    ...
    public static function seperateLangData($data, $langFields): array
    {
        $langData = [];
        $mainData = [];
    ...
    public static function isDataOwner($user, $data): false|JsonResponse
    {
     ...
     * @throws Exception
     */
    public static function convertTimeToUTCzone($str, $userTimezone, $format = 'Y-m-d H:i:s'): string
    {
     ...
     * @throws Exception
     */
    public static function convertTimeToUSERzone($str, $userTimezone, $format = 'Y-m-d H:i:s'): string
    {
        if (empty($str)) {
            return '';
        }
    ...
    public static function idGenerator($model): string
    {
        ...
        return rand(6, 9) . rand(100, 999) . rand(0, 5) . $lastId;
    }
    ...
    public static function formatAddress($address): string
    {
        if ($address) {
            $addressArr = [];
            $addressArr[] = $address->address_1;
            $addressArr[] = $address->address_2;
            $addressArr[] = $address->city;
            $addressArr[] = $address->state;
            $addressArr[] = $address->country;
            $addressArr[] = $address->zip;
    ...
    public static function formatDate($date, $format = 'h:i a, d M, y'): string
    {
    ...
    public static function orderId($item): string
    {
        return self::formatDate($item->created_at, 'Ydm') . $item->id . $item->user_id . $item->status;
    }

    public static function calcPrice($order): array
    {
        $subtotal = 0;
        $shipping_price = 0;
        $tax = 0;
        ...
        $order->load('user_delivery_type.deliveryType');
        if ($order?->user_delivery_type) {
            $shipping_price = $order?->user_delivery_type?->delivery_price;
        }
    ...
    public static function jsDecryption($encrypted): string
    {
    ...
    public static function generateTrackingId($item): string
    {
        $now = Carbon::now();
        return Utils::formatDate($now, 'Ydm') . Utils::generateRandomString(5) . $item['user_id'];
    }

    public static function makeKeyword($string): string
    {
        $removeWords = [
