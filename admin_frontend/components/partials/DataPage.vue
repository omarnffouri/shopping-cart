  ...
  >
    ...
    <div class="form-wrapper">
      ...
      <div v-if="!loading">
        ...
        <img v-else-if="fileKeys.length && $can(gate, 'view')"
          class="mx-w-300x" :src="getImageURL(result.image)" alt="">
      </div>
...
<script>
  ...
  export default {
    setup(){
      ...
      const {getById, setById, setImageById, emptyAllList} = commonStore

      const indexStore = useIndexStore()
      const {mediaStorage} = storeToRefs(indexStore)

      return {getById, setById, setImageById, emptyAllList, mediaStorage}
    },
    name: "DataPage",
      ...
      async uploadFile(file, name = null) {
        try {
          ...
          if (data && (!this.gate || this.$can(this.gate, 'view'))) {
            this.$emit('result', Object.assign({}, data))
      ...
      async checkForm() {
        ...
        try {
          ...
          if (data) {
            ...
            this.$emit('on-success', Object.assign({}, data))
