<template>
      ...
      >
        ...
        <div class="img-wrapper">
          <ImageLazy
            :lazy-src="getThumbImageURL(productImage.image)"
          />
          <button
...
<script>
  ...
  export default {
    setup(){
      ...
      const {allAttributeValues} = storeToRefs(commonStore)
      const {deleteData, setRequest} = commonStore

      return {deleteData, setRequest, allAttributeValues}
    },
    name: 'ProductImage',
    data() {
      return {
    ...
    methods: {
      ...
      async imageDelete(id) {
        if (confirm(this.$t('admin.dltMsg'))) {
          try {
            this.deleting = true
            const data = await this.deleteData({params: id, api: 'deleteProductImage', id: id })
            if (data) {
